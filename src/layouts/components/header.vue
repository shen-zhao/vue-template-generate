<template>
  <div class="headers clearfix">
    ...
    <ul class="layui-nav top_menu right">
      ...
      <li class="layui-nav-item layui-nav-item-last lockcms" pc>
        ...
        <dl class="layui-nav-child layui-anim layui-anim-upbit">
          ...
          <dd style="text-align: center;">设置设置设置设置设置设置设置</dd>
        </dl>
      </li>
    ...
    <el-dialog
      ...
      :show-close="false"
      title="设置设置设置设置设置设置"
      :visible.sync="popDialog.personal.dialogFormVisible"
      width="500px"
    >
      <el-form
        size="small"
    ...
    <el-dialog
      ...
      :show-close="false"
      title="设置设置设置设置"
      :visible.sync="popDialog.password.dialogFormVisible"
      width="500px"
    >
      <el-form
        size="small"
...
export default {
  ...
  methods: {
    ...
    modPersonalInfo(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$http
            .post(this.api.edit, this.popDialog.personal.form)
            .then(response => {
    ...
    modPassword(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$http.post(this.api.edit, psotData).then(response => {
            this.utils.successMsg(this, response.msg);
    ...
    getInfo() {
      this.$http.get(this.api.getInfo).then(response => {
    ...
    // 设置设置
    logout() {
      this.$confirm("设置设置设置设置设置？").then(() => {
        this.$http.get(this.api.logout, {}).then(response => {
          this.utils.successMsg(this, response.msg);
    ...
    // 设置设置设置
    notice() {
      this.$http.get(this.api.notice, {}).then(response => {
        // debugger;
        const data = response.data;
